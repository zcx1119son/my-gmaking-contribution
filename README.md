...
| :--- | :--- |
| **로컬 과부하:** 프로젝트 초기, $\text{GPU}$ 없이 $\text{CPU}$만으로 $\text{Stable Diffusion}$ 이미지를 생성 시도. 이미지 생성에 **과도한 시간(수분)**이 소요되고, 시스템 과부하로 인해 서비스 이용 불가. | **외부 $\text{API}$ 위임:** $\text{AI}$ 이미지 생성 로직을 로컬 $\text{CPU}$에서 **$\text{AI\_Horde}$** 외부 $\text{API}$ 서버로 위임하여 **부하를 분산**하고, $\text{Frontend}$에 결과를 빠르게 반환하는 $\text{MSA}$ 구조를 확립함. |

### 2. ⚡️ AI 서버 통신 지연 및 응답 속도 최적화

$\text{AI\_Horde}$ $\text{API}$ 통신 과정에서 이미지 생성에 소요되는 시간(지연)을 단축하기 위해 **모델과 파라미터를 지속적으로 테스트 및 수정**했습니다.
